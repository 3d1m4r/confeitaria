...
async function processarCheckout(event) {
    ...
    const taxId = formData.get('taxId').replace(/\D/g, '');
    ...
    try {
        ...
        const response = await fetch('https://rest-express-q5x4.replit.dev/api/checkout', {
            method: 'POST',
            headers: {
        ...
        window.currentPixId = result.pixId;
        
        // Resetar botão
        document.getElementById('submit-btn').innerHTML = 'GERAR QR CODE PIX';
...
async function verificarPagamento() {
    ...
    try {
        const response = await fetch(`https://rest-express-q5x4.replit.dev/api/payment/check/${window.currentPixId}`);
        const result = await response.json();
        
        if (result.isPaid) {
            // Pagamento confirmado
            alert('🎉 Pagamento confirmado! Redirecionando para o curso...');
            setTimeout(() => {
                window.location.href = 'https://confeitaria-obrigado.netlify.app/';
            }, 2000);
        } else {
            alert('Pagamento ainda não confirmado. Verifique se o pagamento foi efetuado e tente novamente.');
        }
    } catch (error) {
        console.error('Erro:', error);
        alert('Erro ao verificar pagamento. Tente novamente.');
    } finally {
        btn.innerHTML = '✅ Já efetuei o pagamento';
        btn.disabled = false;
    }
}

// PIX function removed as requested - checkout already handles PIX payments
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // PIX buttons removed as requested
});
